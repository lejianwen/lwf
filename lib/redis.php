...
class redis
{
    ...
    public function attrList($column, $offset = 0, $limit = 5, $desc = true)
    {
        ...
        $offset_end = $limit - 1;
        if ($desc) {
            $list = $this->connect()->client->zRevRange("ph:{$column}:list", $offset, $offset_end);
        } else {
            $list = $this->connect()->client->zRange("ph:{$column}:list", $offset, $offset_end);
        }
        return $list;
    }
    ...
    public function set($key, $value, $ttl = 0)
    {
        ...
        $this->connect()->client->set($key, $value);
        if ($ttl) {
    ...
    {
        $value = $this->connect()->client->get($key);
