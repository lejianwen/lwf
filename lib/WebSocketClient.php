 ...
 * 基于https://github.com/matyhtf/framework/blob/master/libs/Swoole/Client/WebSocket.php
 * 做了自己的修改，方便在fpm中使用
 * @package app\helpers
 */
class WebSocketClient
{
     ...
     * @return bool
     */
    public function connect($type = SWOOLE_SOCK_TCP, $timeout = 0.5)
    {
        if (extension_loaded('swoole')) {
            if ($this->ssl) {
                $type |= SWOOLE_SSL;
...
//            ]);
            ...
            throw new \Exception('swoole not found!');
        }
        //建立连接
     ...
     * @return bool
     */
    protected function doHandShake($headerBuffer)
    {
    ...
    /**
     * 接收数据
     * @return bool | \Swoole\WebSocket\Frame
     * @throws \Exception
     */
    function recv()
    {
        ...
        if ($this->buffer and $offset = stripos($this->buffer, "\r\n\r\n") !== false) {
            ...
            $frame = \Swoole\WebSocket\Server::unpack($packet);
            if ($frame) {
                return $frame->data;
            }
        } else {
            while (true) {
                ...
                $frame = \Swoole\WebSocket\Server::unpack($data);
                if ($frame) {
                    return $frame->data;
                }
            }
